...
from backend.prompts.grounding_critic_prompts import SYSTEM_PROMPT, USER_PROMPT_TEMPLATE
...
def grounding_failure_result(reason: str) -> GroundingCriticResult:
    return GroundingCriticResult(
        grounded=False,
...
def create_grounding_completion(
    client: Any,
    deployment: str,
    ...
    try:
        ...
        wait_for_chat_turn(
            request_id=request_id,
            deployment=deployment,
            purpose="grounding_critic_no_json_mode",
        )
...
def call_grounding_model_with_retries(
    client: Any,
    deployment: str,
    messages: list[dict[str, str]],
    request_id: str | None,
    ...
    for attempt in range(1, max_retries + 2):
        try:
            log_event(
                event="grounding_critic_model_call_started",
                request_id=request_id,
                deployment=deployment,
                attempt=attempt,
            ...
            return create_grounding_completion(
                client=client,
                deployment=deployment,
                messages=messages,
                request_id=request_id,
            ...
            log_event(
                event="grounding_critic_rate_limited",
                level="WARNING",
                request_id=request_id,
                deployment=deployment,
                attempt=attempt,
            ...
            log_event(
                event="grounding_critic_rate_limit_retry_wait_started",
                request_id=request_id,
                deployment=deployment,
                attempt=attempt,
            ...
            log_event(
                event="grounding_critic_rate_limit_retry_wait_completed",
                request_id=request_id,
                deployment=deployment,
                attempt=attempt,
...
def evaluate_grounding(
...
) -> GroundingCriticResult:
    if not answer or not answer.strip():
        return grounding_failure_result("No answer was provided for grounding evaluation.")

    if not context or not context.strip():
        return grounding_failure_result("No retrieved context was provided for grounding evaluation.")

    critic_deployment = get_deployment_for_role(ModelRole.CRITIC)

    log_event(
        event="grounding_critic_started",
        request_id=request_id,
    ...
    with elapsed_timer() as timer:
        ...
        try:
            response = call_grounding_model_with_retries(
                client=client,
                deployment=critic_deployment,
                messages=messages,
                request_id=request_id,
            ...
            result = GroundingCriticResult(**parsed)

            log_event(
                event="grounding_critic_completed",
                request_id=request_id,
                deployment=critic_deployment,
                grounded=result.grounded,
                requiresRevision=result.requiresRevision,
                unsupportedClaimCount=len(result.unsupportedClaims),
                missingCitationCount=len(result.missingCitations),
            ...
            log_event(
                event="grounding_critic_rate_limit_fallback_returned",
                level="WARNING",
                request_id=request_id,
                deployment=critic_deployment,
            ...
            return grounding_failure_result(
                "Grounding critic was temporarily rate-limited and the answer requires review."
            )

        except (APITimeoutError, APIError) as exc:
            log_event(
                event="grounding_critic_api_error",
                level="ERROR",
                request_id=request_id,
                deployment=critic_deployment,
                error=str(exc),
            ...
            return grounding_failure_result(
                "Grounding critic API call failed and the answer requires review."
            )

        except Exception as exc:
            log_event(
                event="grounding_critic_failed",
                level="ERROR",
                request_id=request_id,
                deployment=critic_deployment,
                error=str(exc),
            ...
            return grounding_failure_result(
                "Grounding critic failed and the answer requires review."
            )
